...
def generate_spin_pc_operators(N, L, closed=True, t=None, phi=0., Z_replace=False, Np=None):
    if t is None:
        t_ = [1]
    elif not isinstance(t, list) and not isinstance(t, tuple):
        t_ = [t]
    ...
    for r in range(r_t):
        if t_[r] != 0:
            J_pm = [[t_[r] * term * np.exp(phase)] + [i + j for j in range(r + 2)]
                    for i in range(L - 1 - r)]
            J_mp = [[-t_[r] * term * np.exp(-phase)] + [i + j for j in range(r + 2)]
                    for i in range(L - 1 - r)]
            for k in range(N):
                ops.append(['|' * (N - k - 1) + '+' + 'z' * r + '-' + '|' * k, J_pm])
                ops.append(['|' * (N - k - 1) + '-' + 'z' * r + '+' + '|' * k, J_mp])
            
            if closed and r + 1 < L / 2:
                J_pm = [[t_[r] * term * np.exp(phase)] + [i + j for j in range(L - r)]
                        for i in range(r + 1)]
                J_mp = [[-t_[r] * term * np.exp(-phase)] + [i + j for j in range(L - r)]
                        for i in range(r + 1)]
                for k in range(N):
                    if Z_replace and Np[-k - 1] % 2 == 0:
                        J_pm_ = copy.deepcopy(J_pm)
                        ...
                        J_mp_ = J_mp
                    ops.append(
                        ['|' * (N - k - 1) + '-' + ('I' if Z_replace else 'z') * (L - 2 - r) +
                         '+' + '|' * k, J_pm_])
                    ops.append(
                        ['|' * (N - k - 1) + '+' + ('I' if Z_replace else 'z') * (L - 2 - r) +
                         '-' + '|' * k, J_mp_])
