...
def main(N, L, t, U, V, r, layers, phi_max, phi_num, backend, exact, perturb):
	...
	for l in range(1, layers + 1):
		...
		for i in phi:
			...
			np.savetxt(f"data_vqe/{N}_{L}_Hubbard_{l}_layers_U_{U}_V_{V}_r_{r}_(1,1,1)_CURRENT",
			           persistent_current_vector, delimiter=", ", newline="\n")
			np.savetxt(f"data_vqe/{N}_{L}_Hubbard_{l}_layers_U_{U}_V_{V}_r_{r}_(1,1,1)_ENERGY",
			           energy_vector, delimiter=", ", newline="\n")
			np.savetxt(f"data_vqe/{N}_{L}_Hubbard_{l}_layers_U_{U}_V_{V}_r_{r}_(1,1,1)_ENTROPY",
			           entropies, delimiter=", ", newline="\n")
			np.savetxt(f"data_vqe/{N}_{L}_Hubbard_{l}_layers_U_{U}_V_{V}_r_{r}_(1,1,1)_PARAMETERS",
